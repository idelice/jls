...
public class JavaDebugServer implements DebugServer {
    ...
    private static void createLogFile() {
        try {
            ...
            var logPath = Paths.get(System.getProperty("java.io.tmpdir"), "jls-debug-server.log");
            var logFile = new FileHandler(logPath.toString(), false);
    ...
    public JavaDebugServer(DebugClient client) {
        ...
        class LogToConsole extends Handler {
            ...
            private final ThreadLocal<Boolean> inPublish = new ThreadLocal<>();

            @Override
            public void publish(LogRecord r) {
                if (Boolean.TRUE.equals(inPublish.get())) {
                    return;
                }
                inPublish.set(true);
