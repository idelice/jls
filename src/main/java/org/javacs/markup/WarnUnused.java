...
class WarnUnused extends TreeScanner<Void, Void> {
    ...
    private final Map<Element, TreePath> importDeclarations = new HashMap<>();
    ...
    Set<TreePath> unusedImports() {
        var unused = new HashSet<Element>(importDeclarations.keySet());
        unused.removeAll(used);
        var paths = new HashSet<TreePath>();
        for (var entry : importDeclarations.entrySet()) {
            if (unused.contains(entry.getKey())) {
    ...
    public Void visitImport(ImportTree t, Void __) {
        var qualified = t.getQualifiedIdentifier().toString();
        if (qualified.endsWith(".*")) {
            return null;
        }
        var importPath = new TreePath(path, t.getQualifiedIdentifier());
        var element = trees.getElement(importPath);
        if (element != null) {
            importDeclarations.put(element, new TreePath(path, t));
        }
        return null;
    }
