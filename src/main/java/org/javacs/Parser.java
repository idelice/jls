...
class Parser {
    ...
    static String pruneBody(Path file) {
        var contents = FileStore.contents(file);
        if (contents.contains("lombok")) {
            return contents;
        }
        var parse = parseFile(file);
        ...
        new TreeScanner<Void, Void>() {
            ...
            public Void visitMethod(MethodTree node, Void __) {
                ...
                if (body != null) {
                    var start = (int) positions.getStartPosition(parse.root, body);
                    var end = (int) positions.getEndPosition(parse.root, body);
                    if (start != -1 && end != -1 && start < end) {
                        erase(buffer, start + 1, end - 1);
                    }
                }
                return null;
            }
