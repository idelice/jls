...
public class SignatureProvider {
    ...
    public SignatureHelp signatureHelp(Path file, int line, int column) {
        ...
        try (var task = compiler.compile(file)) {
            ...
            LOG.log(Level.SEVERE, "Signature help failed with compiler assertion", e);
            return NOT_SUPPORTED;
        } catch (RuntimeException e) {
            LOG.log(Level.SEVERE, "Signature help failed", e);
            return NOT_SUPPORTED;
        }
    }
    ...
    private List<ExecutableElement> memberOverloads(CompileTask task, CompilationUnitTree root, MemberSelectTree method) {
        var trees = Trees.instance(task.task);
