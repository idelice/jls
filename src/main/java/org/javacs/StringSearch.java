...
public class StringSearch {
    ...
    public static boolean containsAnyWord(Path java, Collection<String> queries) {
        if (queries == null || queries.isEmpty()) return false;
        if (FileStore.activeDocuments().contains(java)) {
            var text = FileStore.contents(java).getBytes(StandardCharsets.UTF_8);
            for (var query : queries) {
                if (query == null || query.isEmpty()) continue;
                if (new StringSearch(query).nextWord(text) != -1) return true;
            }
            return false;
        }
        try (var channel = FileChannel.open(java)) {
            ...
            channel.read(SEARCH_BUFFER);
            SEARCH_BUFFER.position(0);
            var buffer = SEARCH_BUFFER.asReadOnlyBuffer();
            buffer.position(0);
            for (var query : queries) {
                if (query == null || query.isEmpty()) continue;
                if (new StringSearch(query).nextWord(buffer) != -1) return true;
            }
            return false;
        } catch (NoSuchFileException e) {
            LOG.warning(e.getMessage());
    ...
    static String packageName(String contents) {
        var packagePattern = Pattern.compile("^package +(.*);");
        var startOfClass = Pattern.compile("^[\\w ]*class +\\w+");
        try (var lines = new BufferedReader(new StringReader(contents))) {
            for (var line = lines.readLine(); line != null; line = lines.readLine()) {
                if (startOfClass.matcher(line).find()) return "";
                var matchPackage = packagePattern.matcher(line);
                if (matchPackage.matches()) {
