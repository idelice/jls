...
class ReusableCompiler {
    ...
    ReusableCompiler(boolean disableReuse) {
    ...
    static class ReusableContext extends Context implements TaskListener {
        ...
        void clear() {
            ...
            if (ht.get(Log.logKey) instanceof ReusableLog) {
                ...
                // Recreate JavaCompiler to reset annotation processing state cleanly.
                drop(JavaCompiler.compilerKey);
                put(JavaCompiler.compilerKey, ReusableJavaCompiler.factory);
                try {
                    var procEnv = Class.forName("com.sun.tools.javac.processing.JavacProcessingEnvironment");
                    drop(procEnv);
                } catch (Throwable t) {
                    // ignore if class is not accessible
                }
        ...
        static class ReusableJavaCompiler extends JavaCompiler {
            ...
            @Override
            public void processAnnotations(com.sun.tools.javac.util.List<JCTree.JCCompilationUnit> roots) {
                super.processAnnotations(roots);
                // Prevent reuse of a DeferredDiagnosticHandler with a nulled queue.
                setDeferredDiagnosticHandler(null);
            }
