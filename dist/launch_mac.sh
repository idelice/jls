...
if [[ -n "$WORKSPACE_ROOT" && -r "$RUNTIMES_FILE" ]]; then
    ...
    for gradle_name in ("build.gradle", "build.gradle.kts"):
        ...
        for key in ("sourceCompatibility", "targetCompatibility"):
            ...
            if m:
                ...
                if r:
                    return r
        m = re.search(r"JavaLanguageVersion\.of\((\d+)\)", gradle)
        if m:
            r = parse_release_string(m.group(1))
            if r:
                return r
...
if [[ -n "$LOMBOK_PATH" ]]; then
  if [[ -z "$RUNTIME_JAVA" && -z "$HOST_JAVA_HOME" ]]; then
    echo "Warning: lombokPath set but JAVA_LSP_HOST_JAVA/JAVA_HOME missing; using java from PATH" >&2
  fi
  if [[ -r "$LOMBOK_PATH" ]]; then
    JAVA_ARGS+=("-javaagent:$LOMBOK_PATH")
  else
